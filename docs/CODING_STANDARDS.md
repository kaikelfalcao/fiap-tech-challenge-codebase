...
# Project Style Guide and Coding Conventions

> Reference: [An unofficial TypeScript Style Guide](https://github.com/basarat/typescript-book/blob/master/docs/styleguide/styleguide.md)
...
- **Create a resource for each model**
> Each domain model should have its own module, controller, and service. This keeps the application organized and easier to maintain and test.
```
src/
 └── user/
     ├── user.module.ts
     ...
     └── dto/
         ...
         └── update-user.dto.ts
```
- **Modules must be atomic and have a single responsibility**
> Each module should fully encapsulate the business logic related to a single concept of the system. Avoid modules doing multiple unrelated things.

**Bad**
```ts
AppModule -> handles users, products, orders, etc
```
**Good**
```ts
UserModule -> handles users
ProductModule -> handles products
```
- **Avoid business logic in controllers**
> Controllers should only handle HTTP request/response and delegate to services. Services must contain all the business rules.


## Quick Reference
### 📦 Estrutura de Código
-   **Variáveis/Funções** → `camelCase`
-   **Classes/Interfaces/Types/Namespaces/Enums** → `PascalCase`

-   **Membros de classe/interface** → `camelCase`

-   **Interfaces** → **não** usar prefixo `I`


### 📜 Convenções

-   Arrays → `Type[]` (não `Array<Type>`)

-   Arquivos → `camelCase` (ou `PascalCase` se exportar componente React/NestJS)

-   `type` → quando precisar de _union_ ou _intersection_

-   `interface` → quando usar `extends` ou `implements`
...
-   Preferir **`undefined`** (exceto APIs que pedem `null`)

-   Para checar ambos → usar `if (foo != null)`

-   Para objetos → usar truthy (`if (foo)`)


### 🖊️ Formatação

-   Aspas → `'single'` (usar `backticks` quando string complexa)

-   Espaços → `2` (não tab)

-   Sempre usar `;`

-   Usar formatador automático (`tsfmt`, Prettier ou do VSCode)


### 🛠️ NestJS

-   **1 módulo por recurso** (`user/`, `product/` etc)

-   Módulos → responsabilidade única

-   Controllers → apenas HTTP in/out

-   Services → toda a lógica de negócio

-   DTOs e Exceptions em pastas próprias
